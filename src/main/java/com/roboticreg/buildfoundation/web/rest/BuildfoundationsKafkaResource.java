...
@RestController
@RequestMapping("/api/buildfoundations-kafka")
public class BuildfoundationsKafkaResource {

    private final Logger log = LoggerFactory.getLogger(BuildfoundationsKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public BuildfoundationsKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
