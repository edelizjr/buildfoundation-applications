...
import com.roboticreg.buildfoundation.config.EmbeddedKafka;
import com.roboticreg.buildfoundation.config.KafkaSseConsumer;
import com.roboticreg.buildfoundation.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class BuildfoundationsKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/buildfoundations-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/buildfoundations-kafka/consume")
